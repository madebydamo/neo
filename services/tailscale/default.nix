...
in
{
  ...
  config = mkIf cfg.enabled {
    ...
    system.activationScripts.create-tailscale-files =
      let
        flags =
          [ ]
          ++ (if !cfg.acceptDns then [ "--accept-dns=false" ] else [ ])
          ...
          ++ (if cfg.exitNode != null then [ "--exit-node=${cfg.exitNode}" ] else [ ])
          ++ (if cfg.exitNodeAllowLanAccess then [ "--exit-node-allow-lan-access" ] else [ ])
          ++ (if cfg.hostname != null then [ "--hostname=${cfg.hostname}" ] else [ ])
          ++ (
            if cfg.loginServer != "https://controlplane.tailscale.com" then
              [ "--login-server=${cfg.loginServer}" ]
            else
              [ ]
          )
          ++ (if cfg.ssh then [ "--ssh" ] else [ ]);

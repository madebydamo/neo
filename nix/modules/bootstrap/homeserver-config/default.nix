{...}: {
  flake.modules.nixos.homeserver-config = {
    ...
    rootSettingsPath = ../../../settings.toml;
    settingsContent =
      if builtins.pathExists rootSettingsPath
      then builtins.readFile rootSettingsPath
      else ''
        # settings.toml - copied from Neo root during bootstrap
        [device]
        hostname = "homeserver"
        grubDevice = "/dev/sda"
  ...
  in
    lib.mkIf cfg.enabled {
      ...
      #   ''
      # );
      systemd.services.neo-homeserver-config-bootstrap = lib.mkIf cfg.bootstrapEnable {
        ...
        script = ''
                    ...
                    if [ ! -d "$CONFIG_PATH/.git" ]; then
                      mkdir -p "$CONFIG_PATH"
                      cd "$CONFIG_PATH"
                      if [ "${cfg.bootstrapMethod}" = "clone" ] && [ -n "${safeRepoUrl}" ]; then
                        ${lib.getBin pkgs.git}/bin/git clone --depth 1 "${safeRepoUrl}" . 2>/dev/null || ${lib.getBin pkgs.git}/bin/git clone "${safeRepoUrl}" .
                      else
                        nix flake init -t github:madebydamo/neo#homeserver
                        ${lib.getBin pkgs.git}/bin/git init
                        if [ -n "${safeRepoUrl}" ]; then
                          ${lib.getBin pkgs.git}/bin/git remote add origin "${safeRepoUrl}"
                        fi
                      fi
                      cat > settings.toml << 'EOFTOML'
          ${settingsContent}
          EOFTOML
                      mkdir -p modules
                      cat > modules/hardware.nix << 'EOFHW'
          {lib, ...}: {
            # Hardware configuration - generated by Neo bootstrap
            # Auto-imported by import-tree in the dendritic pattern.
            # Add device-specific config here (e.g. kernel params, pci, etc).
            # Core settings (neo.device etc) come from settings.toml.
          }
          EOFHW
                       nix run .#write-flake
                       ${lib.getBin pkgs.git}/bin/git config user.name "${cfg.gitUserName}"
                       ${lib.getBin pkgs.git}/bin/git config user.email "${cfg.gitUserEmail}"
                       ${lib.getBin pkgs.git}/bin/git add .
                       ${lib.getBin pkgs.git}/bin/git commit -m "Initial bootstrap from Neo template" || true

                      if [ -n "${safeRepoUrl}" ]; then
                        ${lib.getBin pkgs.git}/bin/git branch -M main || true
